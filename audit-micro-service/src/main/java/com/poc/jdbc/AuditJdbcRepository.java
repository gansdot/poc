...
public class AuditJdbcRepository {
	...
	class AuditRowMapper implements RowMapper<Audit> {
		...
		public Audit mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			audit.setCaseNumber(rs.getString("SF_CASE_ID"));
	...
	public List<Audit> findById(String caseNumber) {
		return jdbcTemplate.query("select * from POC.AUDIT where SF_CASE_ID=?", new Object[] { caseNumber },
				new BeanPropertyRowMapper<Audit>(Audit.class));
	}

	public int deleteById(String caseNumber) {
		return jdbcTemplate.update("delete from POC.AUDIT where SF_CASE_ID=?", new Object[] { caseNumber });
	}
	...
	public int insert(Audit audit) {
		return jdbcTemplate.update("insert into POC.AUDIT (SF_CASE_ID, TNX_NAME, TNX_STATUS,"
				...
				new Object[] { audit.getCaseNumber(), audit.getTnxName(), audit.getTnxStatus(),audit.getReqData(), 
	...
	public int update(Audit audit) {
		return jdbcTemplate.update("update POC.AUDIT set TNX_STATUS = ?, RES_DATA = ?, RES_DATETIME = ?"
				...
				new Object[] {audit.getTnxStatus(), audit.getResData(),  audit.getResDatetime(),audit.getCaseNumber(), 
						audit.getTnxName() });
	}
